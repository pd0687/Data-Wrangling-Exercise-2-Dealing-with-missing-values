...
titanic$embarked <- ifelse(titanic$embarked == "C", "C",
                    ifelse(titanic$embarked == "S", "S",
...
mean_age <- titanic %>% summarise(mean_age = mean(age, na.rm = TRUE))
...
### different lifeboats, but as we know, many passengers did not make it to
### a boat :-( This means that there are a lot of missing values in the boat
### column. Fill these empty slots with a dummy value e.g. the string 'None'
### or 'NA.'
...
### value? What does a missing value here mean? You have a hunch that the
### fact that the cabin number is missing might be a useful indicator of
### survival. Create a new column has_cabin_number which has 1 if there is
...
titanic %>% write.csv("titanic_clean.csv")